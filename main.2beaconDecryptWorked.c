...
#define APP_NO_ADV_GAP_TICKS    30000
...
static uint8_t m_prev_adv_msg[APP_AES_LENGTH] = {0};
static uint32_t m_last_timestamp = 0;
                            ...
                            const ble_gap_evt_adv_report_t *p_adv_report)
{
	...
	if (!memcmp(p_data + 9, &m_prev_adv_msg, APP_AES_LENGTH))
		return true;	
	...
	encrypt_128bit_uuid(p_data + 9, m_aes128_key, extracted_uuid, m_counter + counter_tick);
	if (!memcmp(extracted_uuid, m_beacon_uuid, APP_AES_LENGTH))
	{
		memcpy(&m_prev_adv_msg, p_data + 9, APP_AES_LENGTH);
		return true;
	}
	return false;
}
...
static void on_ble_evt(ble_evt_t * p_ble_evt)
{
    ...
    switch (p_ble_evt->header.evt_id)
    {
        case BLE_GAP_EVT_ADV_REPORT:
        {
            ...
            {
				app_timer_cnt_get(&m_last_timestamp);
...
static void timer_timeout_handler (void *p_context)
{
	...
	app_timer_cnt_diff_compute(cur_timestamp, m_last_timestamp, &time_diff);
	err_code = bsp_indication_set(BSP_INDICATE_IDLE);
	APP_ERROR_CHECK(err_code);
	//SEGGER_RTT_printf(0, "time diff: %d\n", time_diff);
	if (time_diff > APP_NO_ADV_GAP_TICKS)
	{
